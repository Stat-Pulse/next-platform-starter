...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Ordered by game_date ASC to show earliest games first
    const [seasonGamesRaw] = await connection.execute(
      `SELECT game_id, week, game_date AS date, game_time,
       ...
       ORDER BY game_date ASC`, // Changed to ASC
      [teamId, teamId]
    );
    ...
    // Ordered by gameday ASC, then gametime ASC for chronological order within a day
    const [upcomingSchedule] = await connection.execute(
      `SELECT game_id,
        ...
        WHERE (home_team = ? OR away_team = ?)
          AND gameday >= CURDATE()
        ORDER BY gameday ASC, gametime ASC`, // Added gametime ASC
      [teamId, teamId]
    );
