...
export default async function handler(req, res) {
  ...
  const teamFullName = teamNameMap[teamAbbr.toUpperCase()] || teamAbbr;
  const query = `${teamFullName} NFL`;
  const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&apiKey=${apiKey}&language=en&sortBy=publishedAt&pageSize=10`;
  ...
  try {
    ...
    res.status(200).json(data.articles.slice(0, 5)); // No filter — trust NewsAPI query
  } catch (error) {
